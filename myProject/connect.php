...
include("/usr/local/uvm-inc/jtbrenna.inc");

$databaseName="JTBRENNA_FINAL";        

$dsn = 'mysql:host=webdb.uvm.edu;dbname=';

function dbConnect($dbName){
    global $db, $dsn, $admin_username, $admin_password;

    if (!$db) $db = new PDO($dsn . $dbName, $admin_username, $admin_password); 
        if (!$db) {
          return 0;
        } else {
          return $db;
